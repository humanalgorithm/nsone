...
class RecordProcessing():
    ...
    def process_record(self, row_data, zone):
        try:
            record, source = yield zone.loadRecord(**self.build_args("load", row_data)), "LOAD"
            record, source = yield record.update(**self.build_args("update", row_data, record.answers)), "UPDATE"
        except ResourceException as e:
            if e.message == "server error: record not found":
                func_name = "add_{}".format(row_data['Type'])
                func = getattr(zone, func_name)
                record, source = yield func(**self.build_args("create", row_data)), "CREATE"
    ...
    def build_args(self, key, row_data, existing_answers=[]):
        ...
        args = data[key]
        updated_answers = self.prepare_answers(existing_answers, key, row_data)
        if updated_answers:
            args['answers'] = updated_answers
        log("record_request_{}".format(key), args)
        return args

    def prepare_answers(self, answers, key, row_data):
        def get_mx_answer():
            answers_update = []
    ...
    def remove_record_from_zone_record_queue(self, row_data):
        ...
        zone_store.zone_record_queues[zone_name] = filter(lambda x: x != row_data, zone_store.zone_record_queues[zone_name])